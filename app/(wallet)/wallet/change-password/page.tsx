  ...
  return (
    <>
      ...
      <div className="animate-rise-in motion-reduce:animate-none motion-reduce:translate-y-0 motion-reduce:opacity-100">
        <SectionCard className="max-w-xl" title="Wallet password" description="Choose a strong password for this mock wallet">
          <form className="space-y-4" onSubmit={form.handleSubmit(submit)}>
            <div className="space-y-2">
              <Label htmlFor="currentPassword">Current password</Label>
              <Input id="currentPassword" type={show ? "text" : "password"} {...form.register("currentPassword")} />
              {form.formState.errors.currentPassword && (
                <p className="text-sm text-wallet-outgoing">{form.formState.errors.currentPassword.message}</p>
              )}
            </div>
            <div className="space-y-2">
              ...
              <Input id="newPassword" type={show ? "text" : "password"} {...form.register("newPassword")} />
                ...
                <div className="space-y-1">
                  <div className="flex gap-1" aria-hidden="true">
                    {Array.from({ length: 4 }, (_, index) => (
                      <span
                        key={index}
                        className={cn("h-1 flex-1 rounded-full transition-colors duration-200", index < score ? STRENGTH[score].className : "bg-secondary")}
                      />
                    ))}
                  </div>
              ...
              {form.formState.errors.newPassword && (
                <p className="text-sm text-wallet-outgoing">{form.formState.errors.newPassword.message}</p>
              )}
            </div>
            <div className="space-y-2">
              <Label htmlFor="confirmPassword">Confirm password</Label>
              <Input id="confirmPassword" type={show ? "text" : "password"} {...form.register("confirmPassword")} />
              {form.formState.errors.confirmPassword && (
                <p className="text-sm text-wallet-outgoing">{form.formState.errors.confirmPassword.message}</p>
              )}
            </div>
