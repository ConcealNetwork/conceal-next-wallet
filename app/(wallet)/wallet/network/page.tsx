...
export default function NetworkPage() {
  ...
  const syncPct = data.networkHeight > 0 ? Math.min((data.height / data.networkHeight) * 100, 100) : 0
    ...
    <>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <ChartCard
          ...
          tone="incoming"
          delay={360}
          chart={<Sparkline values={data.blockTimeHistory} color="hsl(var(--chart-2))" baseline={BLOCK_TARGET_SECONDS} />}
        />
      </div>
    </>
  )
...
function Meta({ label, value, tone, mono }: { label: string; value: string; tone?: "incoming"; mono?: boolean }) {
  return (
    <span className="flex flex-col leading-tight">
      <span className="text-xs text-muted-foreground">{label}</span>
      <span className={cn("text-sm font-semibold", mono && "font-mono", tone === "incoming" && "text-wallet-incoming")}>
        {value}
      </span>
    </span>
  )
...
function SyncRing({ pct }: { pct: number }) {
  ...
  return (
    ...
    >
      <circle cx="85" cy="85" r={radius} fill="none" stroke="hsl(var(--secondary))" strokeWidth="12" />
      <circle
        cx="85"
        cy="85"
        r={radius}
      ...
      />
      <text x="85" y="82" textAnchor="middle" className="fill-foreground font-mono text-[34px] font-bold">
        {Math.round(clamped)}%
      </text>
...
function Sparkline({ values, color, baseline }: { values: number[]; color: string; baseline?: number }) {
  ...
  return (
    <svg viewBox="0 0 100 40" preserveAspectRatio="none" className="h-12 w-full" aria-hidden="true">
      {baselineY !== null ? (
        <line x1="0" y1={baselineY} x2="100" y2={baselineY} stroke="hsl(var(--border))" strokeDasharray="3 4" />
      ) : null}
