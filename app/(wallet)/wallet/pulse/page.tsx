...
export default function PulsePage() {
  ...
  function broadcast() {
    ...
    if (!contact || !hasRelationship(contact) || !paymentIdTo) {
      return toast.error(t("pulse.errNoOutboundPid"));
    }
  ...
  return (
    <>
      <PageHeader title={t("nav.pulse")} subtitle={t("pulse.subtitle")} />
        ...
        <SectionCard title={t("pulse.sendTitle")} description={t("pulse.sendDescription")}>
          <div className="grid gap-3 sm:grid-cols-[2fr_1fr_1fr_1fr_auto] sm:items-end">
            <div className="space-y-2">
              ...
              >
                <option value="">{t("pulse.selectPlaceholder")}</option>
                {connected.map((c) => (
          ...
          {connected.length === 0 ? (
            <p className="mt-3 text-sm text-muted-foreground">{t("pulse.noConnectedContacts")}</p>
          ) : null}
        </SectionCard>
