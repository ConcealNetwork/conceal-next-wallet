...
export default function ReceivePage() {
  ...
  const paymentUri = useMemo(() => buildPaymentUri(address, amount, paymentId, message), [address, amount, paymentId, message])
      ...
      <div className="grid gap-6 xl:grid-cols-[1.6fr_1fr]">
        <div className="space-y-6">
          <div className="animate-rise-in motion-reduce:animate-none motion-reduce:translate-y-0 motion-reduce:opacity-100">
            <SectionCard title="Your Wallet Address" description="Share this address or QR to receive funds">
              <div className="flex flex-col gap-6 sm:flex-row sm:items-start">
                <div className="flex-1 space-y-4">
                  <p className="break-all rounded-xl bg-secondary p-4 font-mono text-sm text-foreground">{address}</p>
          ...
          <div className="animate-rise-in motion-reduce:animate-none motion-reduce:translate-y-0 motion-reduce:opacity-100 [animation-delay:70ms]">
            <SectionCard title="Request a Payment" description="Optionally encode an amount, payment ID, and message">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="req-amount">Amount (CCX)</Label>
                  <Input id="req-amount" type="number" step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="0.00" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="req-paymentId">Payment ID</Label>
                  <Input id="req-paymentId" value={paymentId} onChange={(event) => setPaymentId(event.target.value)} placeholder="Optional" autoComplete="off" />
                </div>
                <div className="space-y-2 sm:col-span-2">
                  <Label htmlFor="req-message">Message</Label>
                  <Textarea id="req-message" value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Optional note for the sender" />
                </div>
              </div>
              {hasRequest ? (
                <div className="mt-4 space-y-3 rounded-xl bg-secondary p-4">
                  <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Payment link</p>
                  <p className="break-all font-mono text-sm text-foreground">{paymentUri}</p>
                  <CopyButton value={paymentUri} label="Copy Payment Link" />
                </div>
              ) : (
                <p className="mt-4 text-sm text-muted-foreground">Fill any field above to generate a shareable payment link and update the QR.</p>
              )}
            </SectionCard>
          </div>
        </div>
