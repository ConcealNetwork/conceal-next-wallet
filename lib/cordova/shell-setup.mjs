...
export const CORDOVA_DEFAULT_PLUGINS = [
  ...
  { id: "cordova-plugin-save-dialog", spec: "^2.0.0" },
  {
    id: "cordova-plugin-biometric-unlock",
    // Native source lives in conceal-wallet-cordova (not this repo). Path is relative
    // to the Cordova project root when `--ensure-plugins` runs. See that repo's
    // cordova-plugins/cordova-plugin-biometric-unlock/README.md and switch.sh.
...
/**
 * True when `spec` is a local path, file: dir, tarball, or remote URL — not an npm
 * version range. Those must be passed to `cordova plugin add` verbatim.
 */
function isLocalOrRemoteSource(spec) {
  return /^(file:|https?:|git@|ssh:|git:|[./]|[a-zA-Z]:[\\/])/.test(spec);
}
...
export function cordovaPluginAddArgs(plugin) {
  let target;
  if (plugin.spec) {
    target = isLocalOrRemoteSource(plugin.spec) ? plugin.spec : `${plugin.id}@${plugin.spec}`;
  } else {
    target = plugin.id;
  }
  const args = ["plugin", "add", target];
  if (plugin.variables) {
