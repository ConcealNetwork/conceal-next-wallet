...
export async function fetchCcxMarketData(): Promise<MarketData> {
  ...
  marketDataPromise = (async () => {
    ...
    try {
      ...
      history30d = memoryCharts.get("30D")?.data ?? readLocal<MarketHistoryPoint[]>("ccx-coingecko-chart-30D-v1")?.data ?? []
    }
...
export async function hydrateMarketHistory(data: MarketData, range: MarketTimeframe): Promise<MarketData> {
