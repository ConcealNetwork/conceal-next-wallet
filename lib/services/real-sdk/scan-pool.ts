 ...
 *
 * SAFETY: every failure mode falls back to the in-thread scan for the affected chunk (no pool / no
 * Worker support / a worker error / a timeout), so the worker path can never break or stall sync —
 * worst case it is exactly as fast as the single-threaded fold. The caller must have run
 * `ensureSdkReady` (the in-thread fallback uses the main-thread WASM).
 *
...
/** Lazily create the worker pool. Returns null when workers are unavailable (SSR / blocked). */
