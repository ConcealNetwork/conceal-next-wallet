...
describe("wallet mappers", () => {
  ...
  it("maps core transactions for the UI without dropping fusion or miner rows", () => {
    ...
    minerOut.rtcAmount = ""
    const miner = mapCoreTransaction(makeTx({ outs: [minerOut], hash: "miner-tx" }), 200, walletAddress)
  ...
  it("maps core deposits to UI deposits with status and unlock progress", () => {
    ...
    expect(deriveIndicativeDepositApr(deposit.amount, deposit.interest, deposit.term, network)).toBeGreaterThan(0)
